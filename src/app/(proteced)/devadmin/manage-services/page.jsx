...
const AdminServices = () => {
    ...
    const fetchSaved = async () => {
        const res = await axios.get(`/api/admin/services?version=${selectedVersion}`);
        console.log(res)
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const res = await axios.get(`${process.env.NEXT_PUBLIC_DATA_API}/api/services`);
                setServices(res.data || []);

                // fetch local admin services
                const saved = await axios.get("/api/admin/services");
                        ...
                        {selectedVersion && currentVersion && (
                            <div className="border p-4 rounded-lg bg-white border-gray-300 shadow-sm flex flex-col gap-5">
                                <h3 className="text-xl font-bold text-gray-800">
                                    Version: <span className="text-[var(--rv-admin-bg-color)]">{currentVersion.slug}</span>
                                </h3>
                                    ...
                                    {currentVersion?.services?.map((srv) => (
                                        <label
                                            ...
                                            className={`group flex items-start gap-3 p-2 border rounded-lg cursor-pointer transition-all ${selected.includes(srv._id.toString())
                                                ? "bg-blue-50 border-[var(--rv-admin-bg-color)]"
                                        ...
                                        >
                                            <div className="relative flex items-center">
                                                <input
                                                    ...
                                                    id={`chk-${srv._id}`}
                                                />
                                                <span className="h-5 w-5 flex items-center justify-center border-2 border-gray-400 rounded-md peer-checked:bg-[var(--rv-admin-bg-color)] peer-checked:border-[var(--rv-admin-bg-color)] transition">
                                                    <svg
                                ...
                                <div className="flex items-start w-full">
                                    <button
                                        className="px-6 py-2 bg-[var(--rv-admin-bg-color)] hover:bg-[var(--rv-admin-bg-color)] text-white font-semibold rounded-lg shadow transition disabled:opacity-50 flex items-center gap-2"
                                        onClick={async () => {
                                            try {
